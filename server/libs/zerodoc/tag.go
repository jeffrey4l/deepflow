...
func GenTagColumns(code Code) []*ckdb.Column {
	...
	if code&TAPPort != 0 {
		...
		columns = append(columns, ckdb.NewColumnWithGroupBy("capture_nic", ckdb.UInt32).SetIndex(ckdb.IndexNone).SetComment("采集位置标识 若capture_network_type为3: 虚拟网络流量源, 表示虚拟接口MAC地址低4字节 00000000~FFFFFFFF"))
	...
	if code&TAPType != 0 {
		columns = append(columns, ckdb.NewColumnWithGroupBy("capture_network_type", ckdb.UInt8).SetComment("流量采集点(1-2,4-255: 接入网络流量, 3: 虚拟网络流量)"))
	}
	if code&VTAPID != 0 {
