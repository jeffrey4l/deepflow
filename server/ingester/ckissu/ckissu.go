		...
		for idx, oldTable := range []string{"vtap_flow_port", "vtap_flow_edge_port", "vtap_app_port", "vtap_app_edge_port"} {
			...
			for _, dsInfo := range datasourceInfos {
				...
				if err := i.renameTable(connection, &TableRename{
					...
					NewTables: []string{strings.Replace(dsInfo.name, oldTable, newTables[idx], 1) + "_agg"},
				}); err != nil {
					return err
				}
				...
				// recreate mvTable,localTable,globalTable
				if err := ds.Handle(strings.Split(newTables[idx], "_")[0], "add", dsInfo.baseTable, dsInfo.name, dsInfo.summable, dsInfo.unsummable, interval, DEFAULT_TTL); err != nil {
					return err
				}
			}
		}
